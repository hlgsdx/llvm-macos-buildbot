...
./test-release.sh -release "$LLVM_RELEASE" -final -triple x86-64-apple-darwin22.0 -no-checkout -no-clang-tools -no-test-suite -no-openmp -no-polly -no-mlir -no-flang -use-gzip -no-compare-files -configure-flags -DLLVM_APPEND_VC_REV=OFF -"$1"
if [ "$1" != "stage3" ]; then
...
echo "file_name=clang+llvm-$LLVM_RELEASE-x86-64-apple-darwin22.0.tar.gz" >> $GITHUB_OUTPUT
echo "release_tag_version=$_release_tag_version" >> $GITHUB_OUTPUT
printf 'SHA512 checksum:\n<code>' > github_release_text.md
printf "$(shasum -a 512 final/clang+llvm-"$LLVM_RELEASE"-x86-64-apple-darwin22.0.tar.gz | sed 's,final/,,' | sed 's, ,\&nbsp;,g')" >> github_release_text.md
printf '</code>\n' >> github_release_text.md
mkdir output
mv final/clang+llvm-"$LLVM_RELEASE"-x86-64-apple-darwin22.0.tar.gz output/
